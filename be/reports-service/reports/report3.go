...
type Report3Result struct {
	PickupZipCode  string `json:"pickup_location"`
	DropoffZipCode string `json:"dropoff_location"`
	CaseCount      string `json:"case_count"`
}

func HandleReport3(w http.ResponseWriter, r *http.Request) {
	...
	var results []Report3Result
	for rows.Next() {
		var r Report3Result
		if err := rows.Scan(&r.PickupZipCode, &r.DropoffZipCode, &r.CaseCount); err != nil {
