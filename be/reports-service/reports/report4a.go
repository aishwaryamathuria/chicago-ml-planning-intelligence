...
type Report4AResult struct {
	PickupNeighborhood string `json:"pickup_neighborhood"`
	TripCount          string `json:"trip_count"`
}

func HandleReport4A(w http.ResponseWriter, r *http.Request) {
	...
	query := `SELECT 
			...
			WHERE (p.ccvi_category = 'HIGH'OR d.ccvi_category = 'HIGH')
				AND pickup_neighborhood IS NOT NULL
				AND pickup_neighborhood::text != '[null]'
	...
	var results []Report4AResult
	for rows.Next() {
		var r Report4AResult
		if err := rows.Scan(&r.PickupNeighborhood, &r.TripCount); err != nil {
